...
async function exportAbi() {
  ...
  const skipNames =
    /(Mock|test_helpers|Imports|deposit_contract|Pausable|.dbg.json|build-info|interfaces)/

  const artifactPaths = allArtifactPaths
    .map((f) => path.relative(artifactsPath, f))
    .filter((relpath) => !skipNames.test(relpath))

  const lidoArtifactPaths = artifactPaths.filter(
    (p) => p.substring(0, 10) === 'contracts/'
  )
...
async function extractABIs(artifactPaths, abisPath) {
  ...
  for (const artifactPath of artifactPaths) {
    const artifactContent = await fs.readFile(
      path.join(artifactsPath, artifactPath)
    )
    ...
    if (artifact.abi && artifact.abi.length) {
      ...
      await fs.writeFile(
        path.join(abisPath, `${artifact.contractName}.json`),
        abiData
      )
    }
  }
