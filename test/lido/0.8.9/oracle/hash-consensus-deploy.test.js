...
const computeSlotAt = (time) =>
  Math.floor((time - GENESIS_TIME) / SECONDS_PER_SLOT)
const computeEpochAt = (time) =>
  Math.floor(computeSlotAt(time) / SLOTS_PER_EPOCH)
const computeEpochFirstSlot = (epoch) => epoch * SLOTS_PER_EPOCH
const computeEpochFirstSlotAt = (time) =>
  computeEpochFirstSlot(computeEpochAt(time))
const computeTimestampAtEpoch = (epoch) =>
...
async function deployHashConsensus(
  ...
  if (!reportProcessor) {
    reportProcessor = await MockReportProcessor.new(CONSENSUS_VERSION, {
      from: admin,
    })
  }
  ...
  if (initialEpoch !== null) {
    ...
    await consensus.setTime(
      genesisTime + initialEpoch * slotsPerEpoch * secondsPerSlot
    )
  }

  await consensus.grantRole(
  ...
  await consensus.grantRole(
    await consensus.MANAGE_FAST_LANE_CONFIG_ROLE(),
    admin,
    { from: admin }
  )
  await consensus.grantRole(
    await consensus.MANAGE_REPORT_PROCESSOR_ROLE(),
    admin,
    { from: admin }
  )
...
contract('HashConsensus', ([admin, member1]) => {
  context('Deployment and initial configuration', () => {
    ...
    it('reverts if admin address is zero', async () => {
      const reportProcessor = await MockReportProcessor.new(CONSENSUS_VERSION, {
        from: admin,
      })
      await assert.revertsWithCustomError(
        HashConsensus.new(
